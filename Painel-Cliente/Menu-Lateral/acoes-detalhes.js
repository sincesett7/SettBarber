// acoes-detalhes.js (VERSÃO FINAL COM PAGINAÇÃO, BOTÃO ATUALIZAR E SUBMENU DINÂMICO)

const API_URL_ACOES = '/api/';
    ...
    paginatedItems.forEach(acao => {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td>
                    <button class="btn-ver-membros" data-members="${membersData}" data-action-name="${acao.nome_da_ação || 'Ação'}">
                        Ver Membros
                    </button>
                </td>
...
function renderTopMembers(actions) {
    ...
    victoriousActions.forEach(acao => {
        ...
        membros.forEach(membro => {
            ...
            if (trimmedMember) {
                memberWins[trimmedMember] = (memberWins[trimmedMember] || 0) + 1;
            }
        });
    });
        ...
        <div class="modal-backdrop">
            <div class="modal-content">
                <div class="modal-header">
                    <h3>Membros da Ação: ${actionName}</h3>
...
async function fetchAcoesData() {
    try {
        ...
        const response = await fetch(API_URL_ACOES, {
            method: 'POST',
            headers: { 'Content-Type': 'text/plain;charset=utf-8' },
            body: JSON.stringify({ action: 'getAcoes' })
        });
        ...
        renderUI(allActions); // Centraliza a renderização da UI

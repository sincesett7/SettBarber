// lavagens-detalhes.js (VERSÃO FINAL COM API E PAGINAÇÃO)

const API_URL_LAVAGENS = '/api/';
...
// Função para formatar números grandes para o submenu (ex: 123.456 -> 123k)
...
function populateKpiCards(ops) {
    ...
    // Atualiza o preview no submenu lateral
...
function renderChart(ops) {
    // Pega a data de 30 dias atrás a partir de hoje
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    thirtyDaysAgo.setHours(0, 0, 0, 0); // Zera a hora para comparar apenas o dia

    // Filtra as operações para incluir apenas as dos últimos 30 dias
    ...
    // Agrupa o lucro da FAC por dia
    const profitByDay = recentOps.reduce((acc, op) => {
        const date = op.data.split(' ')[0]; // Pega apenas a parte da data "dd/MM/yyyy"
    ...
    if (chartInstance) chartInstance.destroy(); // Destrói o gráfico anterior para evitar sobreposição
    ...
    chartInstance = new Chart(ctx, {
        ...
        options: {
            ...
            scales: { y: { ticks: { color: '#A9A5B3' } }, x: { ticks: { color: '#A9A5B3' } } },
...
async function fetchLavagensData() {
    try {
        ...
        const response = await fetch(API_URL_LAVAGENS, {
            method: 'POST',
            headers: { 'Content-Type': 'text/plain;charset=utf-8' },
            body: JSON.stringify({ action: 'getLavagens' })
        });
        ...
        renderChart(allLaundering); // Passa todos os dados para o gráfico
        ...
        const tbody = document.getElementById('lavagens-table')?.querySelector('tbody');
        if(tbody) {
             tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;">Falha ao carregar dados. Erro: ${error.message}</td></tr>`;
        }
    }
}

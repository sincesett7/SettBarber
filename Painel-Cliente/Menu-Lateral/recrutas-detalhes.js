// Arquivo: recrutas-detalhes.js (VERSÃO FINAL COM PAGINAÇÃO E MAIS)

const API_URL = '/api/'; // Use sua URL mais recente
...
function populateKpiCards(recruits) {
    ...
    // Atualiza o contador de pendentes no menu lateral
...
async function fetchData() {
    try {
        ...
        const response = await fetch(API_URL, {
            ...
            body: JSON.stringify({ action: 'getRecrutas' })
        });
        ...
        const tbody = document.getElementById('recrutas-table')?.querySelector('tbody');
        if(tbody) {
             tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;">Falha ao carregar os dados. Erro: ${error.message}</td></tr>`;
        }
    }
}
